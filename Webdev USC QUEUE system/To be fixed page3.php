...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['decrement'])) {
    $conn->query("UPDATE users SET queue_number = queue_number - 1 WHERE id_number = '$user_id' AND queue_number > 1");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['cancel'])) {
    $conn->query("DELETE FROM users WHERE id_number = '$user_id'");
    session_destroy();
